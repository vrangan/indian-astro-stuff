...
def evaluate_year_worker(year):
    ...
    # Target the CE historical file bundle
    ...
    for month in range(1, 13):
        for day in range(1, 32):
            ...
            # FIX: Extract the latitude and longitude components from the tuple properly
            ...
            # Altaz also returns an internal tuple (alt, az, distance), unpack the components
            ...
            # --- RELAXED CONSTRAINTS FOR HIGH RECALL ---
            # 1. Expand Margazhi window to the first 7 days of the month
            is_margazhi_start = (tamil_month == "Margazhi" and (sun_s % 30) <= 7.0)

            # 2. Accept either the day of the full moon or the day immediately leading into it (Tithi 13 or 14)
            is_pournami = (tithi_idx in [13, 14])

            # 3. Allow dawn twilight to scale naturally from astronomical to civil twilight
            is_dawn = (-15.0 < alt_s < -3.0)

            # 4. Loosen Jupiter's setting boundary (visible but setting in the West)
...
if __name__ == '__main__':
    ...
    print("Running Clean Tuple-Unpacked WSL Architecture...")
    print(f"Executing over years {start_year} to {end_year}...\n")
    
    # Batch loops to match 48 core capabilities safely
    chunk_size = 40 
