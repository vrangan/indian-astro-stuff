...
def plot_brahma_muhurtham(c, filename, eph):
    ...
    summary_text = (
        ...
        f"• Sun  : {get_rasi_name(c['sun_lon']).split()[0]} ({c['sun_lon']:.1f}°)\n"
        f"• Moon : {get_rasi_name(c['moon_lon']).split()[0]} ({c['moon_lon']:.1f}°)\n"
    ...
    try:
        ...
        if matches:
            ...
            for rank, c in enumerate(matches[:15], start=1):
                ...
                # FIXED: Formatted the string explicitly from split parameters to prevent list conversion faults
                print(f"{rank:<5} | {c['year']:<5} | {date_part:<24} | {c['sep']:<6.1f} | {get_rasi_name(c['sun_lon']).split()[0]:<12} | {get_nakshatra_name(c['moon_lon']):<12}")
