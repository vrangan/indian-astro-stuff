...
def evaluate_year_chunk(year):
    """Worker function optimized for zero-leak serialization."""
    # Ensure the file path exists locally before reading
    bsp_path = 'de421.bsp'
    ...
    for month in range(1, 13):
        for day in range(1, 32):
            ...
            alt_v = float(srivilliputhur.at(t_dawn).observe(venus).apparent().altaz()[0].degrees)
            alt_j = float(srivilliputhur.at(t_dawn).observe(jupiter).apparent().altaz()[0].degrees)
            alt_s = float(srivilliputhur.at(t_dawn).observe(sun).apparent().altaz()[0].degrees)
            ...
            if is_margazhi_start and is_pournami and is_dawn and planets_aligned:
                # Force every dictionary value into an absolute native Python primitive
                match_data = {
    ...
    # Explicitly close internal file structures before process termination
    eph.close()
    return results
...
if __name__ == '__main__':
    ...
    cores = cpu_count()
    print(f"Initializing Clean Serialization Architecture...")
    print(f"Running engine over {cores} cores for years {start_year} to {end_year}...\n")
    
    with Pool(processes=cores) as pool:
        packed_results = pool.map(evaluate_year_chunk, years_to_scan)
        
    flattened_results = [match for sublist in packed_results if sublist for match in sublist]
