...
# 27 Vedic Nakshatras (13.3333° each)
NAKSHATRA_NAMES = [
...
# Major Chronological Anchors (Yogataras map)
VEDIC_STARS = {
    "Rohini": Star(ra_hours=(4, 35, 55.2), dec_degrees=(16, 30, 33)),
    "Chitra": Star(ra_hours=(13, 25, 11.5), dec_degrees=(-11, 9, 41)),
    "Swati": Star(ra_hours=(14, 15, 39.6), dec_degrees=(19, 10, 56)),
    "Krittika": Star(ra_hours=(3, 47, 24.0), dec_degrees=(24, 7, 0)),
    "Ardra": Star(ra_hours=(5, 55, 10.3), dec_degrees=(7, 24, 25)),
    "Agastya": Star(ra_hours=(6, 23, 57.1), dec_degrees=(-52, 41, 44))
}
...
# ==========================================
# 3. RELAXED HISTORICAL SCAN ENGINE
# ==========================================
    ...
    for year in range(START_YEAR, END_YEAR + 1):
        ...
        for t_moon, phase in zip(t_phases, phases):
            if phase == 2:  # Full Moon Axis
                ...
                # Verify Margazhi Month (Sun in Sagittarius / Dhanu)
                if 240 <= sun_lon < 270:
                    # Map 13th day window approximation
                    ...
                    t_sunrise = t_sunrises[0]  # <-- Extract the single Time scalar object out of the array

                    
                    # Check visibility slightly wider to catch structural variations
                    # Venus should be up in the East, Jupiter low or setting in the West
                    t_check = t_sunrise - timedelta(minutes=45) 
                    obs = observer.at(t_check)
                    alt_v, _, _ = obs.observe(venus).apparent().altaz()
                    alt_j, _, _ = obs.observe(jupiter).apparent().altaz()
                    
                    # Relaxed boundaries to ensure ancient horizon variations are caught
...
# 4. EXPORT ENGINE
# ==========================================
def write_csv_log(candidates, filename, eph):
    earth = eph['earth']
    observer = earth + SRIVILLIPUTHUR
    ...
    print(f"\nGenerated matching dataset log mapping: {filename}")

def plot_brahma_muhurtham(t_sunrise, t_moon, filename, eph):
    # Brahma Muhurtham starts 96 minutes before sunrise
    t_brahma = t_sunrise - timedelta(minutes=96)
    earth = eph['earth']
    observer = earth + SRIVILLIPUTHUR
    
    # Evaluate Day 4 Rain window data
    ...
    ax.axhspan(0, 75, color='#030712', alpha=0.95, label='Sky') 
    ...
    for star_name, star_obj in VEDIC_STARS.items():
        ...
        if -25 <= s_alt.degrees <= 75:
            ax.scatter(s_az.degrees, s_alt.degrees, s=40, c='#fef08a', marker='*', alpha=0.6, zorder=4)
            ax.text(s_az.degrees, s_alt.degrees - 2.5, star_name, ha='center', color='#93c5fd', fontsize=8, alpha=0.8)

    # Plot Planets (Fixed formatting string bugs)
