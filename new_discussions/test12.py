...
def generate_sky_plot(year, month, day, hour_utc, lat, lon, output_filename="sky_chart.png"):
    ...
    # Define directional azimuth labels
    ax.set_xticklabels(['N', 'NE', 'E', 'SE', 'S', 'SW', 'W', 'NW'])
    
    # Limit visualization to objects above the local horizon (Altitude > 0)
    ax.set_ylim(0, 90)
    ax.set_yticklabels([]) # Hide clean rings
    ...
    for name, body in bodies.items():
        ...
        if alt.degrees > 0:
            # Polar charts map radius from center. Zenith (90 deg) is center, horizon (0 deg) is edge
            r = 90 - alt.degrees
    ...
    print(f"Successfully generated sky rendering save file: {output_filename}")
    plt.close()
...
def scan_nammalvar_birth():
    ...
    # Look closely through historical candidate years
    for year in range(790, 805):
        ayanamsa = calculate_ayanamsa(year)
        for month in: # Vaikasi maps to May/June Julian
            for day in range(1, 32):
                try:
                ...
                # Check 3: Relative separation equals a Full Moon (Tithi 15)
                is_full_moon = (168.0 <= (moon_sidereal - sun_sidereal) % 360 <= 192.0)
                
                if is_vaikasi and is_visakha and is_full_moon:
                    print(f" MATCH FOUND: Nammalvar Profile fits Julian Date {year}-{month:02d}-{day:02d}")
                    print(f" -> Sun Sidereal Position: {sun_sidereal:.2f}° | Moon Sidereal Position: {moon_sidereal:.2f}° (Visakha Segment)")
                    eph.close()
                    return
...
if __name__ == '__main__':
    ...
    # Gather up individual json dictionary records
    flattened_results = []
    ...
    # Write findings out directly to a permanent CSV File
    csv_file = "andal_century_scan.csv"
    if flattened_results:
        fields = list(flattened_results[0].keys())
    ...
    # Task 2: Calculate Nammalvar birth dates parameters
    scan_nammalvar_birth()
    
    # Task 3: Render sky graphics configuration for our primary target date (Nov 15, 843 CE)
    # 00:30 UTC corresponds directly to 6:00 AM local early morning twilight
