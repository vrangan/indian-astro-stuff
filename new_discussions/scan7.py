"""
THE ULTIMATE JYOTISHI SCANNER (True Chitra Paksha Edition - V7)
---------------------------------------------------------------
New Constraints:
1. First day of Margazhi (Sun crossing 240.0° exactly).
2. Moon is Full (Purnima) on that exact day.
...
def scan_period(args):
    ...
    try:
        ...
        # Initialize previous day's Sun position to detect the border crossing
        ...
        while curr_t.tt < end_t_val:
            ...
            # 1. BORDER CROSSING: Sun enters Dhanu (Sankramanam)
            if prev_s_long < 240.0 and s_long >= 240.0:
                
                # 2. CHECK PHASE: Is it Purnima today? (168 to 192 covers the whole Tithi)
                m_long = get_vedic_geocentric_pos(curr_t, moon)
                phase = (m_long - s_long) % 360
                
                if 168.0 <= phase <= 192.0: 
                    
                    # 3. ALIGNMENT CHECK: Scan +/- 2 days around Sankramanam
                    ...
                    # Jump forward to skip the rest of this month
...
if __name__ == '__main__':
    freeze_support()
    print(f"--- FIRST DAY MARGAZHI SCANNER (V7) ---")
    ...
    csv_name = f"first_day_margazhi_{START_YEAR}_{END_YEAR}.csv"
