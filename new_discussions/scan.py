"""
THE ULTIMATE JYOTISHI SCANNER (True Chitra Paksha Edition - V3)
---------------------------------------------------------------
Criteria:
...
def scan_period(args):
    ...
    # FIX: Use 'earth' to build the full barycentric vector chain
    earth = eph['earth'] 
    
    # Create the complete Topocentric observer chain
    ...
    # FIX: Pass the fully combined observer_topo to almanac
    vis_venus = almanac.risings_and_settings(eph, observer_topo, venus)
    vis_jupiter = almanac.risings_and_settings(eph, observer_topo, jupiter)
    ...
    while curr_t.tt < end_t_val:
        ...
        if 270.0 <= s_long <= 300.0:
            ...
            # WIDENED WINDOW to catch the step
...
if __name__ == '__main__':
    freeze_support()
    print(f"--- TRUE JYOTISHI SCANNER (V3) ---")
