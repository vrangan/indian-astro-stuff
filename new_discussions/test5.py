...
def evaluate_year_worker(year):
    """
    Skyfield is imported inside the function scope to prevent the global 
    namespace from holding open unpickleable stream connections.
    """
...
if __name__ == '__main__':
    # FORCE Python to launch a pristine new process environment for each core
    ...
    print(f"Initializing Isolated Spawn Architecture...")
    print(f"Processing calculations on {cores} cores for years {start_year} to {end_year}...\n")
    
    with multiprocessing.Pool(processes=cores) as pool:
        # pool.map now distributes simple integers across isolated process trees
        pool.map(evaluate_year_worker, years_to_scan)
        
    # Re-gather data back from the completed disk file outputs
    flattened_results = []
