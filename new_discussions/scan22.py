        ...
        for t_moon, phase in zip(t_phases, phases):
            if phase == 2:  # Full Moon
                ...
                if 238 <= sun_lon <= 272:
                    ...
                    # FIX applied here: Extract index [0] to yield a scalar time value
                    ...
                    # Evaluates safely using your expanded threshold constraints
                    if alt_v.degrees > -2 and -10 < alt_j.degrees < 25:
                        candidates.append((t_sunrise, t_moon))
                        print(f"MATCH FOUND: {t_sunrise.utc_jpl()} | V_Alt: {alt_v.degrees:.1f}°, J_Alt: {alt_j.degrees:.1f}°")
    ...
    try:
        ...
        if matches:# Export chronological dataset records
            ...
            for t_sunrise, t_moon in matches:
                ...
                date_part = raw_date.replace("A.D. ", "").split()[0]
