...
def scan_nammalvar_birth():
    """Scans the late 8th century using flexible multi-hour sweeps to capture fast-moving lunar transitions."""
    ...
    for year in range(790, 805):
        ayanamsa = calculate_ayanamsa(year)
        for month in (5, 6): # Sweep full May/June Julian windows
            for day in range(1, 32):
                # Run three snapshots per day (Dawn, Noon, Dusk) to capture fast moon transits
                for hour in (6.0, 12.0, 18.0):
                    try:
                        t = ts.utc(year, month, day, hour - 5.5) # Map local hours back to UTC
                    except ValueError:
                        continue
                    ...
                    # 1. Sun in Sidereal Taurus (Vaikasi Month: 30° to 60°)
                    is_vaikasi = (30.0 <= sun_sidereal <= 60.0)

                    # 2. Relax Visakha buffer slightly to capture the crossover day (Visakha core is 200° to 213.33°)
                    is_visakha = (198.5 <= moon_sidereal <= 215.0)

                    # 3. Calculate Tithi (Pournami is index 14)
                    separation = (lon_moon.degrees - lon_sun.degrees) % 360
                    tithi_idx = int(separation / 12.0)
                    is_full_moon = (tithi_idx == 14)

                    if is_vaikasi and is_visakha and is_full_moon:
                        # Extract the exact textual Nakshatra name from your global array safely
                        nak_name = NAKSHATRAS[int(moon_sidereal / (360.0 / 27.0))]
                        print(f" SUCCESSFUL MATCH FOUND: Nammalvar Profile fits Julian Date {year}-{month:02d}-{day:02d}")
                        print(f" -> Checking Hour: {hour}:00 Local Time")
                        print(f" -> Sun Sidereal Position: {sun_sidereal:.2f}° (Vaikasi)")
                        print(f" -> Moon Sidereal Position: {moon_sidereal:.2f}° (Mapped Star: {nak_name})")
                        print(f" -> Tithi Index: {tithi_idx} (Pournami Full Moon verified)")
                        eph.close()
                        return
    print("No perfect matches hit. Try loosening constraints further.")
    eph.close()
