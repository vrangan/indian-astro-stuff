"""
THE ULTIMATE JYOTISHI SCANNER (True Chitra Paksha Edition - V5)
---------------------------------------------------------------
Fix: Bypasses internal Skyfield vector-addition bugs by using 
custom topocentric visibility functions.
"""
...
def scan_period(args):
    ...
    # We can use standard load() now since the filename is correct locally
    try:
        eph = load(eph_file)
    except Exception as e:
        return ("ERROR", f"Failed to load ephemeris: {str(e)}")
    ...
    # --- BULLETPROOF VISIBILITY FUNCTIONS ---
    # These replace almanac.risings_and_settings to avoid vector crashes
    def venus_vis(t):
        alt, _, _ = observer_topo.at(t).observe(venus).apparent().altaz()
        return alt.degrees > -0.5667 # Standard atmospheric refraction for rising/setting
    ...
    while curr_t.tt < end_t_val:
        ...
        if 270.0 <= s_long <= 300.0:
            ...
            if 170.0 <= phase <= 190.0: 
                ...
                # 3. Check Alignment using our safe functions
                ...
                # Skip 20 days forward to avoid the same Full Moon
...
if __name__ == '__main__':
    freeze_support()
    print(f"--- TRUE JYOTISHI SCANNER (V5) ---")
